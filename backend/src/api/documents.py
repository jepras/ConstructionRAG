...
async def upload_email_pdf(
    ...
    try:
        ...
        for file in files:
            ...
            # Service returns document_id and storage_url; collect ids
            ...
            public_url = created["storage_url"]
            document_data.append({"document_id": document_id, "filename": file.filename, "storage_url": public_url})
...
async def get_project_documents(
    ...
    try:
        ...
        # Get total count
        count_result = db.table("documents").select("id", count="exact").eq("project_id", str(project_id)).execute()
...
async def get_project_document(
    ...
    try:
        ...
        if document.get("index_run_id"):
            index_run_result = db.table("indexing_runs").select("*").eq("id", document["index_run_id"]).execute()
