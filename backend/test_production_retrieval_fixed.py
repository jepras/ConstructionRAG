#!/usr/bin/env python
"""
Production-exact retrieval comparison tool with HNSW fix verification.
Tests HNSW vs Python similarity exactly as implemented in production.

Usage:
    python test_production_retrieval_fixed.py "Your query here" [--run-id RUN_ID]
"""
...
class ProductionRetrievalTester:
    ...
    async def test_hnsw_exact(self, query: str, top_k: int = 15) -> List[Dict[str, Any]]:
        ...
        """
        logger.info("🔍 Testing HNSW (production exact with fix)...")
        ...
        try:
            ...
            rpc_params = {
                ...
                'match_threshold': -999.0,  # Very low threshold to get all results
            ...
            for result in results:
                # Use the similarity from the function if available
                if 'similarity' in result:
                    similarity = result['similarity']
                else:
                    # Calculate actual cosine similarity as fallback
...
<body>
    <h1>🔍 Retrieval Method Comparison <span class="status {'fixed' if len(hnsw_results) >= 15 else 'issue'}">{'HNSW FIXED' if len(hnsw_results) >= 15 else 'HNSW ISSUE'}</span></h1>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 50%;">HNSW Search ({len(hnsw_results)} results)</th>
                <th style="width: 50%;">Python Similarity ({len(python_results)} results)</th>
            </tr>
        </thead>
        <tbody>
"""
...
"""
        ...
        # Add status message
        if len(hnsw_results) >= 15:
            html_content += """
        <br>
        <div style="background-color: #e8f5e9; padding: 10px; border-radius: 5px; margin-top: 10px;">
            <strong style="color: #4CAF50;">✅ HNSW function is working correctly!</strong><br>
            The function now returns the expected number of results.
        </div>
"""
        else:
            html_content += f"""
        ...
        <div style="background-color: #ffebee; padding: 10px; border-radius: 5px; margin-top: 10px;">
            <strong style="color: #f44336;">⚠️ HNSW function may still have issues</strong><br>
            Expected 15 results but got {len(hnsw_results)}. This could be due to:
            <ul style="margin: 5px 0;">
                <li>The similarity threshold filtering</li>
                <li>The actual number of similar chunks</li>
                <li>Index not fully rebuilt after migration</li>
            </ul>
        </div>
"""
...
async def main():
    ...
    print(f"\n{'='*60}")
    print("PRODUCTION RETRIEVAL COMPARISON TEST (WITH FIX)")
    print(f"{'='*60}")
    ...
    print("Running HNSW search (with fixed function)...")
    ...
    # Check if HNSW is fixed
    if len(hnsw_results) >= args.top_k:
        print("\n✅ HNSW APPEARS TO BE FIXED!")
        print(f"   Getting {len(hnsw_results)} results as expected")
    else:
        print(f"\n⚠️ HNSW may still have issues")
        print(f"   Expected {args.top_k} but got {len(hnsw_results)}")
